...
class AdaRRT():
    ...
    def _get_random_sample(self):
        ...
        :returns: A vector representing a randomly sampled point in the search
            space.
        With probability 0.2, sample near the goal.
        With probability 0.8, sample uniformly.
        """ 
        if np.random.rand() < 0.2:
            return self._get_random_sample_near_goal()
        else:
    ...
    def _check_for_completion(self, node):
        ...
        """
        # FILL in your code here
        dist = np.linalg.norm(node.state - self.goal.state)
    ...
    def _trace_path_from_start(self, node=None):
        ...
        path = []
        current_node = node if node is not None else self.goal
    ...
    def _get_random_sample_near_goal(self):
        """
        Samples a point within +0.05/-0.05 of the goal in each joint dimension.
        The sample is clipped to stay within joint limits.
        """
...
def main(is_sim):
    ...
    delta = 0.25
    eps = 0.2 # Value changed for part 5 
